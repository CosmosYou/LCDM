...
class CVAE_NEW(nn.Module):
    ...
    def __init__(self, config): 
        ...
        self.gc_layers = nn.ParameterList(
            [GraphConv(self.n_nodes, self.n_nodes, config.device).to(self.device) for i in range(self.n_dec_layers)]
                ...
                # graph convolution layers
        self.gc_layers2 = nn.ParameterList(
            [GraphConv(self.n_nodes, self.n_nodes, config.device).to(self.device) for i in range(self.n_dec_layers)]
        )
    ...
    def decode(self, z_input, c_input):
        ...
        if self.layernum_gc == 2:
            gc_out2 = [torch.sigmoid(self.gc_layers2[i](gc_out[i])) for i in range(self.n_dec_layers)]   
            bmm_out = [torch.bmm(gc_out2[i].unsqueeze(2), gc_out2[i].unsqueeze(1)).view(-1, self.n_nodes*self.n_nodes, 1) \
...
class CVAE(nn.Module):
    ...
    def encode(self, x_input,c_input):
        if self.add_c == True:
            x_c_input = torch.cat((x_input.view(-1,self.n_nodes*self.n_nodes), F.one_hot(c_input.to(torch.int64),4).to(torch.float32)),-1).to(self.device) # 加c
            output = F.relu(self.encoder(x_c_input))
        else:
            output = F.relu(self.encoder_no_c(x_input))
    ...
    def decode(self, z_input, c_input):
        z_c_input = torch.cat((z_input.view(-1,self.latent_dim), F.one_hot(c_input.to(torch.int64),4).to(torch.float32)), -1).to(self.device)
        dec_out = [torch.sigmoid(self.dec_layers[i](z_c_input)) for i in range(self.n_dec_layers)]    

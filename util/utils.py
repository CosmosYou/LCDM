...
# for LSGM. To view a copy of this license, see :
# https://github.com/NVlabs/LSGM/blob/main/LICENSE
# ---------------------------------------------------------------
import logging
import os
import math
...
def kl_call(log_q, neg_log_p):
    kl_diag = torch.mean(torch.sum(neg_log_p + log_q, dim=[2, 3]), dim=0)
    kl_all = torch.sum(neg_log_p + log_q, dim=[1, 2, 3])

    # kl_all = torch.stack(kl_all, dim=1)   # batch x num_total_groups
    kl_vals = torch.mean(kl_all, dim=0)   # mean per group

    return kl_all, kl_vals, kl_diag
...
def get_channel_multiplier(num_scales):
